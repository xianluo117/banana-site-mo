...
const state = {
  ...
  cardSize:
    (typeof localStorage !== "undefined" &&
      localStorage.getItem("gem_card_size")) ||
    "md",
  cardDensity:
    (typeof localStorage !== "undefined" &&
      parseInt(localStorage.getItem("gem_card_density") || "3", 10)) ||
    3,
};
...
function isLocalFileUri(uri) {
  ...
  try {
    ...
    return (
      u.origin === window.location.origin && u.pathname.startsWith("/files/")
    );
  } catch {
    return false;
  }
...
async function mapWithConcurrency(items, limit, mapper) {
  ...
  const workers = Array.from({ length: Math.min(cap, arr.length) }, () =>
    worker(),
  );
...
function loadApiConfig(index) {
  ...
  if (cfg) {
    ...
    persistBaseUrlForFormat(
      cfg.apiFormat || state.apiFormat || "gemini",
      dom.baseUrl.value,
    );
    ...
    if (cfg.apiFormat) {
      ...
      if (state.apiFormat === "openai") {
        ...
        if (
          !dom.baseUrl.value ||
          dom.baseUrl.value.includes("generativelanguage.googleapis.com")
        ) {
        ...
        if (
          !dom.baseUrl.value ||
          dom.baseUrl.value.includes("api.openai.com")
        ) {
...
function renderApiConfigs() {
  ...
  configs.forEach((config, index) => {
    ...
    item.className =
      "flex items-center justify-between bg-gray-800/50 p-2 rounded-lg text-xs";
    item.innerHTML = `
...
function renderCloudApiConfigs(list = [], limit = 2) {
  ...
  if (items.length === 0) {
    ...
    empty.className =
      "text-[10px] text-gray-500 bg-gray-800/30 border border-gray-800/60 rounded-lg p-2";
...
async function refreshCloudApiConfigsList() {
  ...
  try {
    const data = await apiFetchJson(
      `/api/api-configs/list?apiFormat=${encodeURIComponent(apiFormat)}`,
    );
    ...
    err.className =
      "text-[10px] text-red-200 bg-red-900/10 border border-red-900/30 rounded-lg p-2";
...
async function saveCloudApiConfig() {
  ...
  const config = {
    ...
    vertexKeysRaw: String(
      localStorage.getItem("gem_vertex_keys") ||
        dom.vertexKeysInput?.value ||
        "",
    ).trim(),
  };

  await apiFetchJson("/api/api-configs/save", {
    method: "POST",
    json: { apiFormat, name: String(name).trim(), config },
  });
...
async function loadCloudApiConfig(id) {
  if (!requireLoginFor({ type: "cloudApiConfigs" })) return;
  const apiFormat = normalizeApiFormat(state.apiFormat || "gemini");
  const data = await apiFetchJson("/api/api-configs/load", {
    method: "POST",
    json: { apiFormat, id },
  });
  ...
  dom.baseUrl.value =
    String(cfg.baseUrl || "").trim() || defaultBaseUrlForFormat(fmt);
  ...
  if (fmt === "vertex") {
    if (cfg.vertexLocation) {
      state.vertexLocation =
        cfg.vertexLocation === "us-central1" || cfg.vertexLocation === "global"
          ? cfg.vertexLocation
          : "global";
...
async function deleteCloudApiConfig(id) {
  ...
  const apiFormat = normalizeApiFormat(state.apiFormat || "gemini");
  await apiFetchJson("/api/api-configs/delete", {
    method: "POST",
    json: { apiFormat, id },
  });
  await refreshCloudApiConfigsList();
...
function updateAuthUI() {
  ...
  if (dom.authUserLabel)
    dom.authUserLabel.textContent = authed ? runtime.me.username : "guest";
  if (dom.authAdminBadge)
    dom.authAdminBadge.classList.toggle("hidden", !isAdmin());

  if (authed) {
    refreshCloudApiConfigsList().catch(() => {});
  } else if (dom.cloudApiConfigContainer) {
    dom.cloudApiConfigContainer.classList.add("hidden");
  }
}
...
async function ensureInlineImageWebP(inlineData) {
  ...
  try {
    ...
    const qInt =
      typeof state.config?.webpQuality === "number" &&
      state.config.webpQuality > 0 &&
      state.config.webpQuality <= 100
...
async function fileUriToInlineData(fileUri) {
  ...
  const key = fileUri;
  if (runtime.fileUriToInlineCache.has(key))
    return await runtime.fileUriToInlineCache.get(key);
    ...
    for (const part of parts) {
      ...
      if (inlineData && inlineData.data && !part.__webpOptimized) {
        try {
          ...
          console.warn(
            "Failed to optimize inline image to WebP, fallback to original.",
            err,
          );
        }
      }
...
function packCollectionSessions(sessions = []) {
  ...
  const packedSessions = sessions.map((sess) => {
    ...
    const packedMessages = safeMessages.map((msg) => {
      const parts = (msg.parts || []).map((part) => {
        ...
        if (inlineData && inlineData.data) {
          ...
          const key =
            getPartImageKey(part) ||
            `inline:${inlineData.mime_type || inlineData.mimeType || "image/png"}:${normalizeBase64(inlineData.data).slice(0, BASE64_COMPARE_LENGTH)}`;
    ...
    return {
      ...
      systemInstruction: sess.systemInstruction,
    ...
    const restoredMessages = (sess.messages || []).map((msg) => {
      const parts = (msg.parts || []).map((part) => {
        ...
        if (
          ...
          fileData.file_uri.startsWith("asset:")
        ) {
          ...
          if (asset && asset.data) {
            return {
              inline_data: {
                mime_type: asset.mime || "image/png",
                data: asset.data,
              },
            };
          }
...
document.addEventListener("DOMContentLoaded", async () => {
  state.apiFormat = normalizeApiFormat(
    localStorage.getItem("gem_api_format") || "gemini",
  );
  dom.baseUrl.value = getBaseUrlForFormat(state.apiFormat);
  if (localStorage.getItem("gem_ver"))
    dom.apiVersion.value = localStorage.getItem("gem_ver");
  if (localStorage.getItem("gem_key"))
    dom.apiKey.value = localStorage.getItem("gem_key");
  if (localStorage.getItem("gem_model"))
    dom.modelId.value = localStorage.getItem("gem_model");
  state.vertexLocation =
    localStorage.getItem("gem_vertex_location") || "global";
  state.collections = JSON.parse(
    localStorage.getItem("gem_collections_v1.0") || "[]",
  ); // 从 localStorage 加载合集
  ...
  const savedModelPrefillQuick = localStorage.getItem(
    "gem_model_prefill_quick",
  );
  ...
  if (dom.systemPromptQuick) {
    dom.systemPromptQuick.addEventListener("input", (e) => {
      ...
      localStorage.setItem(
        "gem_sys_prompt_quick",
        state.quickSystemInstruction,
      );
    });
  }
  ...
  dom.builderRowInput.addEventListener("change", (e) =>
    handleFileUpload(e.target.files, "row"),
  );
  dom.chatFileInput?.addEventListener("change", (e) =>
    handleFileUpload(e.target.files, "chat", state.activeSessionId),
  );
...
function setupDragDrop() {
  ...
  window.addEventListener("dragenter", (e) => {
    ...
    if (!e.target.closest("#builderMessages"))
...
async function handleFileUpload(files, target, rowIdx = null) {
  ...
  const dataUrls = await mapWithConcurrency(
    list,
    4,
    async (file) => await readFileAsDataURL(file),
  );

  const storedItems = new Array(dataUrls.length).fill(null);
...
function setupConfigListeners() {
  ...
  // Thinking
  document
    .getElementById("includeThoughtsCheck")
    .addEventListener("change", (e) => {
      state.config.includeThoughts = e.target.checked;
      updateConfigPreview();
    });
  document
    .getElementById("includeImageConfigCheck")
    .addEventListener("change", (e) => {
      state.config.includeImageConfig = e.target.checked;
      updateConfigPreview();
    });
  document
    .getElementById("includeSafetySettingsCheck")
    .addEventListener("change", (e) => {
      state.config.includeSafetySettings = e.target.checked;
      updateConfigPreview();
    });
  document
    .getElementById("thinkingBudgetRange")
    .addEventListener("input", (e) => {
      state.config.thinkingBudget = parseInt(e.target.value);
      document.getElementById("thinkingBudgetVal").textContent =
        state.config.thinkingBudget;
      updateConfigPreview();
    });
  ...
  // Safety
  ["safeHarassment", "safeHate", "safeSex", "safeDanger", "safeCivic"].forEach(
    (id) => {
  ...
  document.getElementById("apiFormatGroup").addEventListener("click", (e) => {
    ...
    if (state.apiFormat === "openai") {
      dom.apiVersion.value = "v1";
      dom.baseUrl.value =
        getBaseUrlForFormat("openai") || defaultBaseUrlForFormat("openai");
      persistBaseUrlForFormat("openai", dom.baseUrl.value);
    } else if (state.apiFormat === "vertex") {
      dom.apiVersion.value = "v1beta1";
      dom.baseUrl.value =
        getBaseUrlForFormat("vertex") || defaultBaseUrlForFormat("vertex");
      persistBaseUrlForFormat("vertex", dom.baseUrl.value);
    } else {
      dom.apiVersion.value = "v1beta";
      dom.baseUrl.value =
        getBaseUrlForFormat("gemini") || defaultBaseUrlForFormat("gemini");
      persistBaseUrlForFormat("gemini", dom.baseUrl.value);
    }
    localStorage.setItem("gem_ver", dom.apiVersion.value);
    updateLiveEndpoint();
  ...
  if (vertexLocationGroup) {
    vertexLocationGroup.addEventListener("click", (e) => {
      ...
      state.vertexLocation =
        val === "us-central1" || val === "global" ? val : "global";
      localStorage.setItem("gem_vertex_location", state.vertexLocation);
...
function setupImageConfigControls() {
  ...
  applyGroupActiveStyles(
    "aspectRatioGroup",
    state.config.imageConfig?.aspectRatio || "auto",
  );
}
...
function applyGroupActiveStyles(groupId, value) {
  ...
  group.querySelectorAll("button[data-value]").forEach((btn) => {
    ...
    const baseClasses =
      "px-3 py-1.5 rounded-lg border text-xs font-medium transition-colors active:scale-95 flex items-center";
...
function updateApiFormatUI() {
  ...
  if (dom.vertexKeysContainer) {
    dom.vertexKeysContainer.classList.toggle(
      "hidden",
      state.apiFormat !== "vertex",
    );
  }
  ...
  try {
    ...
    if (dom.apiKey) {
      ...
      dom.apiKey.placeholder = isVertex
        ? "Vertex 模式不需要（使用 Vertex Keys）"
        : "粘贴密钥...";
    }
  } catch {
    // ignore
  ...
  if (state.apiFormat === "vertex") {
    const loc =
      state.vertexLocation === "us-central1" ||
      state.vertexLocation === "global"
        ? state.vertexLocation
        : "global";
...
function loadAdvancedConfig() {
  try {
    ...
    const includeSafetyEl = document.getElementById(
      "includeSafetySettingsCheck",
    );
    ...
    if (includeImageEl)
      includeImageEl.checked = state.config.includeImageConfig;
    if (includeSafetyEl)
      includeSafetyEl.checked = state.config.includeSafetySettings;
    ...
    applyGroupActiveStyles(
      "aspectRatioGroup",
      state.config.imageConfig.aspectRatio,
    );
  } catch (e) {}
}
...
function syncInputToBuilder() {
  if (state.promptBuilder.length <= 1) {
    ...
    if (state.promptBuilder.length === 0 && text) {
      ...
      state.promptBuilder.length === 1 &&
      state.promptBuilder[0].role === "user"
    ) {
...
function isBuilderWorkflowActive() {
  ...
  return state.promptBuilder.some(
...
function togglePromptModal(show) {
  ...
  if (show) {
    ...
    if (state.promptBuilder.length === 0) {
      state.promptBuilder.push({
        role: "user",
        text: dom.promptInput.value,
        images: cloneImages(state.images),
      });
    } else if (
      singleUser &&
      (!state.promptBuilder[0].images ||
        state.promptBuilder[0].images.length === 0) &&
      state.images.length
    ) {
      state.promptBuilder[0].images = cloneImages(state.images);
      if (!state.promptBuilder[0].text)
        state.promptBuilder[0].text = dom.promptInput.value;
    }
...
function renderBuilder() {
  ...
  state.promptBuilder.forEach((msg, idx) => {
    ...
    dropZone.addEventListener("drop", (e) => {
      ...
      dropZone.classList.remove("builder-drop-active");
      if (e.dataTransfer.files.length > 0)
        handleFileUpload(e.dataTransfer.files, "row", idx);
    });
...
window.addBuilderMessage = (role) => {
  ...
  renderBuilder();
  dom.promptBuilderContainer.scrollTop =
    dom.promptBuilderContainer.scrollHeight;
};
window.removeBuilderMessage = (idx) => {
  state.promptBuilder.splice(idx, 1);
  renderBuilder();
};
...
window.updateBuilderMessage = (idx, val) => {
  state.promptBuilder[idx].text = val;
};
...
async function runBatchGeneration() {
  ...
  if (useBuilder) {
    ...
    contents = state.promptBuilder.map((msg) => {
      ...
      msg.images.forEach((img) => {
        if (img.file_uri) {
          parts.push({
            file_data: { mime_type: img.mime, file_uri: img.file_uri },
          });
        } else {
  ...
  try {
    const hasSyntheticModelText = contents.some(
      (c) =>
        c.role === "model" &&
  ...
  for (let i = 0; i < count; i++) {
    ...
    const previewText =
      contents.find((c) => c.role === "user")?.parts.find((p) => p.text)
        ?.text || "Image input";
...
async function executeSessionTurn(
  ...
  if (cardStatus) {
    if (isRetry) {
      cardStatus.className =
        "mr-2 flex items-center gap-1 text-[10px] text-blue-400 font-mono";
    ...
    try {
      ...
      if (state.apiFormat === "openai") {
        ...
        if (session.systemInstruction) {
          oaiMessages.push({
            role: "system",
            content: session.systemInstruction.parts
              .map((p) => p.text)
              .join(""),
          });
        }
        ...
        const preparedMessages = await prepareMessagesForRequest(
          session.messages,
        );
        ...
        preparedMessages.forEach((msg) => {
          ...
          (msg.parts || []).forEach((part) => {
            ...
            if (
              fileData &&
              fileData.file_uri &&
              !isLocalFileUri(fileData.file_uri)
            ) {
              content.push({
                type: "image_url",
        ...
        if (session.systemInstruction)
          payload.systemInstruction = session.systemInstruction;
        ...
        const preparedContents = await prepareMessagesForRequest(
          session.messages,
        );
        ...
        if (session.systemInstruction)
          finalPayload.systemInstruction = session.systemInstruction;
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        try {
          ...
          throw new Error(
            `API returned non-JSON response (${status}): ${text.substring(0, 200)}...`,
          );
        }
      }
      ...
      if (!res.ok) {
        ...
        if (status === 429 && attempt < MAX_429_ATTEMPTS) {
          if (cardStatus) {
            ...
            cardStatus.className =
              "mr-2 flex items-center gap-1 text-[10px] text-yellow-400 font-mono";
          ...
          if (Array.isArray(content)) {
            ...
            while ((match = regex.exec(content)) !== null) {
              ...
              if (imageUrl.startsWith("data:")) {
                const [mime_type, base64_data] = imageUrl
                  .substring(5)
                  .split(";base64,");
      ...
      if (newContent) {
        ...
        if (cardStatus) {
          cardStatus.className =
            "mr-2 flex items-center gap-1 text-[10px] text-green-400 font-mono";
  ...
  if (cardStatus) {
    cardStatus.className =
      "mr-2 flex items-center gap-1 text-[10px] text-red-400 font-mono";
...
async function persistGeneratedImagesInSession(sessionId) {
  // cloud image persistence removed
}
...
function updateGridLayout() {
  ...
  switch (d) {
    ...
    case 2:
      className =
        "grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5 max-w-[1600px] mx-auto";
      break;
    case 3:
      className =
        "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-5 max-w-[2000px] mx-auto";
      break;
    case 4:
      className =
        "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 max-w-[2200px] mx-auto";
      break;
    case 5:
    default:
      // 最紧凑：尽量多列
      className =
        "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 md:gap-4 max-w-[2400px] mx-auto";
      break;
  }
...
function renderLibrary() {
  ...
  const items =
    activeLibraryTab === "presets"
      ? state.presets
      : activeLibraryTab === "chats"
        ? state.savedChats
        : state.collections;

  if (items.length === 0) {
    dom.libraryList.innerHTML =
      '<div class="text-center text-gray-500 py-10">没有找到项目。</div>';
    return;
  }
  ...
  items.forEach((item, idx) => {
    const div = document.createElement("div");
    div.className =
      "flex items-center justify-between p-3 hover:bg-gray-800/50 rounded-lg border border-transparent hover:border-gray-700 transition-all group mb-2";
    ...
    if (activeLibraryTab === "presets") {
      const actionRow = div.querySelector(
        "div.flex.items-center.gap-2.opacity-100",
      );
      if (actionRow) {
        const exportBtn = document.createElement("button");
        exportBtn.type = "button";
        exportBtn.className =
          "px-3 py-1.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-200 text-xs rounded font-medium active:scale-95";
        ...
        actionRow.insertBefore(
          exportBtn,
          actionRow.firstChild?.nextSibling || actionRow.firstChild,
        );
      }
    }
...
function presetToMarkdown(preset) {
  ...
  const builder = Array.isArray(preset?.promptBuilder)
    ? preset.promptBuilder
    : [];
  if (builder.length) {
...
function exportPresetAsMarkdown(index) {
  ...
  if (!preset) return;
  const baseName =
    (preset?.name || `preset_${preset?.id || Date.now()}`)
      .toString()
      .replace(/[\\/:*?"<>|]+/g, "_")
...
async function trySaveFavorite(type, item) {
  ...
  const res = await apiFetchJson("/api/favorites/add", {
    method: "POST",
    json: { type, item },
  });
...
async function saveCurrentSessionToLibrary() {
  ...
  const safeBaseName =
    (name || "").replace(/[\\/:*?"<>|]+/g, "_").trim() || `chat_${chatItem.id}`;
  exportImagesForSessions(
    [state.activeSessionId],
    `chat_${safeBaseName}`,
  ).catch((err) => console.error("导出对话图片失败", err));
}
...
async function loadLibraryItem(idx) {
  if (activeLibraryTab === "presets") {
    ...
    document.getElementById(`status-${newId}`).innerHTML =
      `<span class="text-gray-500">已恢复</span>`;
  } else {
    // collections
...
async function deleteLibraryItem(idx) {
  ...
  if (isAuthed()) {
    ...
    if (item?.id != null) {
      try {
        await apiFetchJson(
          `/api/favorites/${activeLibraryTab}/${encodeURIComponent(String(item.id))}`,
          { method: "DELETE" },
        );
      } catch (e) {
...
function handleLibraryImport(e) {
  ...
  reader.onload = (ev) => {
    try {
      ...
      if (json.collections)
        state.collections = [...json.collections, ...state.collections];
      localStorage.setItem("gem_presets_v3.7", JSON.stringify(state.presets));
      localStorage.setItem("gem_chats_v3.7", JSON.stringify(state.savedChats));
      localStorage.setItem(
        "gem_collections_v1.0",
...
function openChat(id) {
  ...
  renderChatAttachments();
  state.sessions[id].messages.forEach((msg) =>
    renderChatMessage(msg.role, msg.parts),
  );
...
function toggleChatDrawer(open) {
  if (open) dom.chatDrawer.classList.remove("translate-x-full");
    ...
    dom.chatDrawer.classList.add("translate-x-full");
...
async function sendChatMessage() {
  ...
  attachments.forEach((img) => {
    if (img?.file_uri) {
      ...
      parts.push({
        inline_data: { mime_type: img.mime || "image/png", data: img.data },
      });
    }
  });
...
function renderChatAttachments() {
  ...
  list.forEach((img, idx) => {
    ...
    wrap.className =
      "relative group w-16 h-16 rounded-lg border border-gray-700 bg-black/20 overflow-hidden";
    wrap.innerHTML = `
...
function renderChatMessage(role, parts) {
  ...
  bubble.className =
    role === "user"
      ? "bg-purple-900/30 border border-purple-800 text-gray-200 rounded-2xl rounded-tr-sm px-4 py-3 max-w-[90%] md:max-w-[85%]"
      : "bg-gray-800/50 border border-gray-700 text-gray-300 rounded-2xl rounded-tl-sm px-4 py-3 max-w-[90%] md:max-w-[85%] prose prose-invert prose-sm";
...
function stopSession(id) {
  ...
  if (statusEl) {
    statusEl.className =
      "mr-2 flex items-center gap-1 text-[10px] text-yellow-500 font-mono font-semibold stopped-status";
...
function toggleSelection(id, checked) {
  ...
  document
    ...
    .classList.toggle("opacity-0", !checked);
  dom.selectedCount.textContent = state.selectedSessions.size;
  dom.selectionToolbar.classList.toggle(
    "translate-y-32",
    state.selectedSessions.size === 0,
  );
  dom.selectionToolbar.classList.toggle(
    "opacity-0",
    state.selectedSessions.size === 0,
...
function deleteSelected() {
  state.selectedSessions.forEach(deleteSession);
  clearSelection();
}
...
async function saveSelectedSessions() {
  ...
  const name = prompt(
    `为 ${count} 个对话输入一个集合名称:`,
    `集合 ${new Date().toLocaleTimeString()}`,
  );
  if (!name) return;
  ...
  if (!isAuthed()) {
    ...
    try {
      ...
      alert(
        "保存失败：数据体积过大。请尝试减少图片数量，或使用“库→导出”备份到文件。",
      );
    }
  }
  ...
  exportImagesForSessions(selectedIds, `collection_${safeBaseName}`).catch(
    (e) => console.error("导出合集图片失败", e),
  );
...
function toggleRawModal(show) {
  dom.rawModal.classList.toggle("hidden", !show);
}

function preloadImage(url) {
  if (!url || typeof url !== "string") return Promise.resolve();
  if (runtime.imagePreloadCache.has(url))
    return runtime.imagePreloadCache.get(url);
...
function openLightbox(src, previewSrc = null) {
  ...
  const preview =
    previewSrc && typeof previewSrc === "string" ? previewSrc : null;
...
function navigateLightbox(direction) {
  ...
  let currentIndex = globalImages.findIndex(
    (img) =>
  ...
  if (currentIndex === -1) {
    currentIndex = globalImages.findIndex(
      (img) => img.src === state.currentLightboxSrc,
    );
  ...
  state.lightboxContext = {
    type: "cards",
    items: globalImages.map((img) => img.src),
    index: nextIndex,
  };
          ...
          if (inlineData && inlineData.data) {
            ...
            if (url.startsWith("data:")) {
              try {
                ...
                zip.file(`save image/${id}_${i}.${extFromMime}`, data, {
                  base64: true,
                });
              } catch (e) {
              ...
              const promise = fetch(url)
                ...
                .catch((err) => {
                  console.error(
                    `Fetch failed for ${url}, saving URL as .txt file instead. Error:`,
                    err,
                  );
    ...
    if (modelListEl) {
      if (apiFormat === "openai") {
        ...
        modelListEl.innerHTML = (data.models || [])
          .map(
            (m) => `<option value="${m.name.replace("models/", "")}"></option>`,
          )
          .join("");
      }
    }
